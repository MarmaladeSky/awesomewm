...
-- Put volume in a container
local apw_container = wibox.container.background(APW)
apw_container.forced_width = 40


local battery_widget = require("battery-widget")
-- local battery_container = wibox.container.background(battery_widget)
-- nbattery_container.forced_width = 40
...
globalkeys = gears.table.join(
    ...
    -- Layout
--    awful.key({ modkey }, "o", function() mykeyboardlayout.next_layout() end,
--              {description = "show the menubar", group = "launcher"})
)
...
local mylockscreen = awful.key({ modkey }, "l", function ()
    awful.spawn("sh -c 'i3lock --pass-media --pass-screen --pass-power --pass-volume --bar-indicator --keylayout 1 & sleep 1 && xset dpms force off'")
...
local brightnessKeys = gears.table.join(
...
local flameshot = awful.key({ "Mod1", "Shift" }, "F4", function ()
    awful.spawn("sh -c 'flameshot gui'")
end, {description = "lock screen and turn off display", group = "custom"})

-- Add the keybinding to the globalkeys table
globalkeys = gears.table.join(
    globalkeys,
    mylockscreen,
    volumeKeys,
